...
import { Panel, FormGroup, FormControl, ControlLabel, Button, Checkbox, ButtonToolbar } from 'react-bootstrap';
...
export default class Ask extends React.Component {
  ...
  async ask() {
    ...
    try {
      let { userId } = this.props,
        { question, anonymously } = this.state,
        res = await createQuestion(userId, question, { anonymously });
  ...
  render() {
    const { userId, isAuthenticated, username } = this.props,
      { isAsked, isLoading, anonymously, question, status } = this.state;
